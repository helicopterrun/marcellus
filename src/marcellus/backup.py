...
from marcellus.config import Settings, _discover_yaml_path
from marcellus.db import open_sidecar

log = logging.getLogger(__name__)

DB_NAME = "marcellus.db"
# Pre-rename DB filename. A backup made before the rename still has its DB
# stored under this name; restore falls back to it when DB_NAME isn't present.
LEGACY_DB_NAMES = ("frigate-sidecar.db",)
...
def _backup_db_name(names: dict[str, Any]) -> str:
    """Return whichever of DB_NAME / LEGACY_DB_NAMES is present in a
    manifest's file list, preferring the current name."""
    for candidate in (DB_NAME, *LEGACY_DB_NAMES):
        if candidate in names:
            return candidate
    raise BackupError(f"manifest lists no sidecar DB file (expected {DB_NAME!r})")
...
def restore_backup(settings: Settings, src: Path, *, force: bool = False) -> BackupManifest:
    ...
    with _read_dir(src) as src_dir:
        ...
        if not force:
            raise BackupError(
                "refusing to restore: stop marcellus first, then re-run with "
                "--force (restoring under a running service corrupts the WAL)"
            )

        db_name = _backup_db_name(manifest.files)
        db_path = Path(settings.sidecar.db_path)
